...
このサイトに書くすべての文章（ブログ記事・ページの文言・meta description等）は、書いたあとに必ず以下の編集プロンプトを自分に適用し、AIくささを消してから出力すること。2026-07-11にユーザーから指示。
...
### HTMLページへの適用の解釈
- 見出しタグ・テーブル・カードなどページの構造はそのまま。禁止されるのは「本文プローズ内」の装飾（<strong>の乱用、——、「」・（）・→・／の多用、コロン区切り）。
- 一人称は「僕」で統一。
