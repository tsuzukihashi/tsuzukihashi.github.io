...
(function () {
  ...
  const ARROW_SVG = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M13 6l6 6-6 6"/></svg>';
  ...
  function formatPrice(app) {
    ...
    return app.formatted_price || app.price || 'Free';
  }

  /**
   * 紹介ページ（LP）への導線。lp_url を持つアプリだけに出す
   */
  ...
  /**
   * リンク複数持ちのカード（Android版あり、または紹介ページあり）を生成。
   * カード全体を <a> で包めないので div にして、リンクを個別に置く。
   */
  function renderRichCard(app) {
    var androidBtn = app.google_play_url
    ...
    return '<div class="app-card-store app-card-multiplatform">' +
      '<div class="app-header">' +
        ...
        '<div class="app-info">' +
          ...
          '<p class="app-meta">' + formatPrice(app) + '</p>' +
        '</div>' +
  ...
  function renderAppCard(app) {
    if (app.google_play_url || app.lp_url) {
      return renderRichCard(app);
    }
